...
def speak(text, tts, save_audio=True):
    """Generate and play speech using Coqui TTS"""
    print(f"\n🔊 Kabu says: '{text}'")
    
    try:
        # Create output folder if it doesn't exist
        output_folder = "coqui_audio_files"
        ...
        if save_audio:
            ...
            output_file = os.path.join(output_folder, f"{timestamp}_{clean_text}.wav")
        else:
            output_file = "kabu_temp.wav"
...
def interactive_mode(tts):
    ...
    print(f"📁 Files will be saved to: coqui_audio_files/")
    ...
    while True:
        ...
        if text.lower() in ['quit', 'exit', 'q']:
            # Show summary
            if os.path.exists("coqui_audio_files"):
                files = [f for f in os.listdir("coqui_audio_files") if f.endswith('.wav')]
                print(f"\n📊 Summary: {len(files)} audio files saved")
                print(f"📁 Location: {os.path.abspath('coqui_audio_files')}")
        ...
        if text.lower() == 'sample':
            ...
            for i, phrase in enumerate(sample_phrases, 1):
                ...
                speak(phrase, tts, save_audio=save_mode)
                time.sleep(0.5)  # Small pause between phrases
            print("\n✅ All samples played!")
            if save_mode:
                print(f"📁 All audio files saved to: coqui_audio_files/")
            continue
        
        speak(text, tts, save_audio=save_mode)
